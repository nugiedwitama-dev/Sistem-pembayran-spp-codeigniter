<main role="main" class="ml-sm-auto px-md-4">
  ...
  <div class="card">
    <div class="card-body">
      <div class="row">
        <div class="col-md-6">
          <div class="alert alert-info" role="alert"><i class="fas fa-info"></i> Masukan NIS / NAMA siswa yang sudah terdaftar untuk melihat pembayaran spp.</div>
          <?= form_open(''); ?>
          ...
          <div class="table-responsive">
            <table class="table">
              <tr>
                ...
                <td style="font-size: 12px;">No.Bayar</th>
                <td style="font-size: 12px;">Tanggal Bayar</th>
                <td style="font-size: 12px;">Jumlah</th>
                <td style="font-size: 12px;">Tagihan</th>
                <td style="font-size: 12px;">Keterangan</th>
                <td style="font-size: 12px;">Bayar</th>
              </tr>
              <?php $no = 1; foreach($spp as $s) : ?>
                 <tr>
                   ...
                   <?php if($s['ket'] == 'lunas'){
                      echo '0';
                    }else if($s['ket'] == 'cicilan'){
                      echo $s['nominal']*50/100;
                    }else{
                      echo $s['nominal'];
                    }?>
                  </td>
                   ...
                   <td>
                    <?php if($s['no_bayar'] == null) : ?>
                     <a href="<?= base_url('dashboard/bayar/' . $siswa['nis'] . '/' . $s['id_spp']); ?>" class="btn btn-info">Bayar</a>
                     <a href="<?= base_url('dashboard/cicil/' . $siswa['nis'] . '/' . $s['id_spp']); ?>" class="btn btn-success">Cicil</a>
                     <?php else : ?>
                      <a href="<?= base_url('dashboard/batal/' . $siswa['nis'] . '/' . $s['id_spp']); ?>" class="btn btn-danger">Batal</a>
                      <a href="<?= base_url('dashboard/cetak/' . $siswa['nis'] . '/' . $s['id_spp']); ?>" target="_blank" class="btn">Cetak</a>
                    <?php endif; ?>
                    <?php if ($s['ket'] == 'cicilan') : ?>
                      <a href="<?= base_url('dashboard/bayar/' . $siswa['nis'] . '/' . $s['id_spp']); ?>" class="btn btn-info">Lunasi</a>
                      <?php endif; ?>
                   </td>
                 </tr>
              <?php endforeach; ?>
